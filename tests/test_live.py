...
One chat call, one embedding call, one image call, the Jev reference call, and one call per public API.
...
def test_jev_reference_call():
    from campus_copilot.decisions.jev import smoke

    settings = live_settings()
    if not settings.has_jev:
        pytest.skip("no TypeSafe key")
    decision, info = smoke(settings)
    assert decision.ok and 0.0 <= decision.noul("urgency") <= 1.0
    assert decision.model.startswith("jev-")
