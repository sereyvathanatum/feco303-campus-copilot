...
| P14 | Laya decision model, CamTech sources, node debug view, tutorial | done (Laya replay fixtures and live Laya reference rows not yet recorded) | Jev replaced by Laya (in-process or `laya-serve`, no key; stub decider when absent); sources are `CamTech-Prospectus.pdf` and `Academic_Info.md` with new probes, re-grounded eval cases, demo turns 1-2, and checkpoints 2-6; `--debug-nodes`, `/nodes`, `retrieve --stages`; `TUTORIAL.md`; experiment references regenerated offline on 2026-09-24 |

## Next action

All phases P0-P14 are done; Laya replay fixtures (`data/fixtures/laya/`) and live Laya reference rows still need a recording run. Open items for the course team:
- Decisions in plan §15: hardware for the Laya checkpoints, hosting (template repository or Classroom), capstone reuse,
