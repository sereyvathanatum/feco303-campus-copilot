...
| P12 | verify script, budgets, release notes | in progress | |

## Next action

P12: `scripts/verify.py` (fresh venv -> install -> offline pytest -> demo -> eval -> optional live suite ->
pass/fail table), `tests/test_live.py` (`-m live`: one NIM chat, one NIM embed, one image call, jev-smoke, one call
per public API), budget check against plan §11 (`docs/budgets.md`), `verify@build` sweep, CHANGELOG entry.
