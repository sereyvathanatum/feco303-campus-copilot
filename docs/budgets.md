...
mode: the decision model over its hosted API (now Laya, see docs/verify-at-build.md), NIM embeddings, chat through Google AI Studio (NIM chat requests were timing out that
...
|---|---|---|---|
| Laya requests per turn | 1 (guard + route + arguments) + 1 parallel passage batch (RAG only) + at most 1 answer check | exactly that: 1 turn request; 4 passage requests in parallel (about 0.4 s for the batch); 1 answer-check batch; 2.4 Laya requests per turn on average over a mixed session | met |
...
| RAG turn with a live chat model | (not budgeted separately) | 70-100 s, almost all in the Gemma 4 call on the AI Studio free tier (Laya share about 1.1 s) | provider-bound; offline and cached runs are unaffected |
| Reranking (added in 1.1.0) | not budgeted | NIM `llama-nemotron-rerank-vl-1b-v2`: about 0.6 s for 20 candidates, one request per RAG turn; the passage filter then asks Laya about the top 5 (was 4) | adds about 0.6 s to a RAG turn |
...
| Laya cost per seat | ≈ 0.05 USD per seat (200 turns); ≈ 2 USD per 40-seat cohort | 3,771 tokens per turn; at the hosted price then in force (0.042 USD per million input tokens, output free) that was 0.00012 USD per turn, 0.023 USD per seat, 0.92 USD per cohort | obsolete: Laya runs on local hardware, `prices.laya` is 0, and the trace report bills no tokens for it; the remaining cost is the forward pass |
